...
export namespace etc {
    export function hsl_to_rgb01_h6(h360: number): number {
        const a = (h360 / 60) % 6;      // -> -6.0 .. 6.0
        const b = a >= 0 ? a : a + 6;   // -> 0.0 .. 6.0 
        return b;
    }
    export function hsl_to_rgb01_lmdl(h6: number): number {
    ...
    export function hsl_to_rgb01_rgb(h6: number, min: number, mdl: number, max: number): number[] {
        const d6 = Math.floor(h6);
        mdl = min + mdl;
        switch (d6) {
    ...
    export function rgb01_to_hsl_sat(min: number, max: number, sZero: number): number {
        const sd = 1 - Math.abs(min + max - 1);
        return sd > sZero ? (max - min) / sd : 0;
    }

    export function rgb01_to_hsl_hue(r: number, g: number, b: number, min: number, max: number, sZero: number): number {
        const d = max - min;
        if (d < sZero) {
            return 0;
        }
...
export function rgb01_to_hsl(rgb: number[]): number[] {
    ...
    const sZero = 0.005
    const l = (min + max) / 2;
    const s = etc.rgb01_to_hsl_sat(min, max, sZero);
    const h = etc.rgb01_to_hsl_hue(r, g, b, min, max, sZero) * 60;
    return [h, s, l];
}
...
export function hsl_to_rgb01(hsl: number[]): number[] {
    const h6 = etc.hsl_to_rgb01_h6(hsl[0]), s = hsl[1], l = hsl[2];
    const lmax = l + s / 2;
    const lmin = l - s / 2;
    const lmdl = lmin == lmax ? lmin : (lmax - lmin) * etc.hsl_to_rgb01_lmdl(h6);
    const rgb = etc.hsl_to_rgb01_rgb(h6, lmin, lmdl, lmax);
    return rgb;
}
...
export const rgb01_to_xyz = etc.composite_2f(srgb_to_lrgb, lrgb_to_xyz);
export const xyz_to_rgb01 = etc.composite_2f(xyz_to_lrgb, lrgb_to_srgb);
...
function overflow(nn: number[], min: number, max: number): boolean {
    for (let i = 0; i < nn.length; i++) {
        if (nn[i] < min || nn[i] > max) {
            return true;
        }
    }
    return false;
}
export const overflow01 = (nn: number[]): boolean => overflow(nn, 0, 1);
export const overflow255 = (nn: number[]): boolean => overflow(nn, 0, 255);
